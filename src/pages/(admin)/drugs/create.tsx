...
export default function CreateDrugPage() {
    ...
    const handleCropComplete = async (blob: Blob) => {
        setIsUploading(true);
        const file = new File([blob], `${formData.slug || "drug"}.jpg`, { type: "image/jpeg" });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await api.post("/api/drugs", formData);
                ...
                <main className="flex-1 overflow-auto">
                    <div className="flex h-16 items-center justify-between border-b px-6 bg-card sticky top-0 z-10 backdrop-blur-sm">
                        <div className="flex items-center gap-4">
                            ...
                            <h2 className="text-sm font-medium tracking-tight uppercase text-muted-foreground">Tambah Obat</h2>
                        </div>
                        <ModeToggle />
                    </div>
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        <label className="text-sm font-bold uppercase text-muted-foreground tracking-wider">Nama Obat</label>
                                        <Input placeholder="Contoh: Amoxicillin" value={formData.name} onChange={(e) => handleNameChange(e.target.value)} className="rounded-xl h-12" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-sm font-bold uppercase text-muted-foreground tracking-wider">Slug (URL)</label>
                                        <Input placeholder="amoxicillin" value={formData.slug} onChange={(e) => setFormData({ ...formData, slug: e.target.value })} className="rounded-xl h-12 font-mono text-sm" />
                                    </div>
                                </div>

                                <div className="space-y-2">
                                    <label className="text-sm font-bold uppercase text-muted-foreground tracking-wider">Gambar Obat (16:9)</label>
