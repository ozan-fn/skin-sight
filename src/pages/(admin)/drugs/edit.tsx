...
export default function EditDrugPage() {
    ...
    useEffect(() => {
        const fetchDrug = async () => {
            try {
                ...
                const drug = res.data.data.find((d: any) => d.id === id);
                if (drug) {
                    setFormData({
                        name: drug.name,
                        slug: drug.slug,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await api.patch(`/api/drugs/${id}`, formData);
                ...
                <main className="flex-1 overflow-auto">
                    <div className="flex h-16 items-center justify-between border-b px-6 bg-card sticky top-0 z-10 backdrop-blur-sm">
                        <div className="flex items-center gap-4">
                            ...
                            <h2 className="text-sm font-medium tracking-tight uppercase text-muted-foreground">Edit Obat</h2>
                        </div>
                        <ModeToggle />
                    </div>
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        ...
                                        <Input placeholder="Nama Obat (Contoh: Amoxicillin)" value={formData.name} onChange={(e) => handleNameChange(e.target.value)} className="rounded-xl h-12" />
                                    </div>
                                    <div className="space-y-2">
